...
export default function CityAdmin() {
  ...
  function handleUpdate(city) {
    setEditingId(city._id); // mark which city we are editing
    setFormData({
      name: city.name,
      cityPoster: city.cityPoster,
      stateId: city.stateId?._id || "", // Use the state ID for dropdown value
    });
    setShowForm(true);
  }
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, cityPoster: e.target.value })
                }
                required
                className="border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-teal-500 outline-none"
              />
        ...
        >
          <table className="w-full max-w-6xl mx-auto bg-white border border-gray-200 shadow-md rounded-xl overflow-hidden">
            <thead className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white">
              <tr>
                <th className="py-4 px-6 text-left font-semibold">City Name</th>
