...
export default function cityAdmin() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get("http://localhost:5500/api/city");
    ...
    fetchData();
  }, [change]); // Kept original dependency array [change]
  ...
  async function deletecity(id) {
    let confrm = confirm("Are you sure you want to delete this city?");
    if (confrm) {
      setChange(true);
      await axios.delete(`http://localhost:5500/api/city/${id}`);
      setcity(city.filter((item) => item._id !== id));
      setChange(false);
    }
  }
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setChange(true);
    await axios.post("http://localhost:5500/api/city", {
      name: e.target[0].value,
      cityPoster: e.target[1].value,
      stateId: e.target[2].value,
    });
    setChange(false);
    setShowForm(false);
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-6">
        ...
        <motion.div // <-- Added motion
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500 mt-2">
              Fill out the details below to add a new city to the database.
            </p>
          </div>
          <form
      ...
      <motion.div // <-- Added motion for container
        ...
        <table className="w-full max-w-4xl bg-white border border-gray-200 shadow-lg rounded-xl overflow-hidden">
          ...
          <tbody>
            {city.map((item, index) => (
              <motion.tr // <-- Added motion for rows, changed animation
                key={item._id}
